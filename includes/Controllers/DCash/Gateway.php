...
class Gateway extends \WC_Payment_Gateway {
	...
	public function initFormFields(): void {
		...
		$this->form_fields = array(
			'enabled'     => array(
				'title'   => __( 'Enable/Disable', 'integrate-dcash-with-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable', 'woocommerce' ),
				'default' => 'no',
			),
			'sandbox'     => array(
				'title'   => __( 'Sandbox', 'integrate-dcash-with-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable sandbox mode for testing payments.', 'woocommerce' ),
				'default' => 'no',
			),
			'api_key'     => array(
				'title'   => __( 'API Key', 'integrate-dcash-with-woocommerce' ),
				'type'    => 'password',
				'default' => '',
			),
			'merchant'    => array(
				'title'       => __( 'Merchant (Store name)', 'integrate-dcash-with-woocommerce' ),
				'type'        => 'text',
				'description' => __( 'The merchant name to show above the DCash QR Code.', 'integrate-dcash-with-woocommerce' ),
				'default'     => 'Merchant',
			),
			'description' => array(
				'title'       => __( 'Description', 'integrate-dcash-with-woocommerce' ),
				'type'        => 'textarea',
				'description' => __( 'The text to show when the customer selects the DCash payment method.', 'integrate-dcash-with-woocommerce' ),
	...
	public function payment_fields() {
		...
		if ( empty( $api_key ) ) {
			if ( current_user_can( 'manage_options' ) ) {
				echo "<p style='margin-bottom: 0;'>" . esc_html__( 'Please enter your DCash Merchant API Key to accept DCash payments.', 'integrate-dcash-with-woocommerce' ) . '</p>';
				return;
			} else {
				echo "<p style='margin-bottom: 0;'>" . esc_html__( 'Payment method not available yet, please check back later.', 'integrate-dcash-with-woocommerce' ) . '</p>';
				return;
			}
		}
		...
		if ( ! empty( $payment_id ) ) {
			$description = $this->get_option( 'description' ) ?: __( 'Pay using your DCash wallet.', 'integrate-dcash-with-woocommerce' );
			echo "<p style='margin-bottom: 0;'>" . esc_html( $description ) . '</p>';
		} else {
			echo "<br/><p style='font-weight: bold'>" . esc_html__( 'There was an issue setting Session Data. Please refresh the page and try again.', 'integrate-dcash-with-woocommerce' ) . '</p>';
			return;
		}
		...
		if ( Functions::sandboxModeEnabled() ) {
			...
			$memo = __( 'Online Payment from WooCommerce Store. Payment ID:', 'integrate-dcash-with-woocommerce' ) . ' ' . $payment_id;
		}
		 ...
		 */
		?>
			<input id='integrate-dcash-with-woocommerce-payment-id' type='hidden' name='dcash_for_wc_payment_id' value='<?php echo esc_attr( $payment_id ); ?>'/>
			<?php wp_nonce_field( 'dcash_for_woocommerce_payment_id', 'integrate-dcash-with-woocommerce-payment-id-nonce' ); ?>
	...
	public function process_payment( $order_id ) {
		...
		$dcash_payment_id_nonce = sanitize_text_field( wp_unslash( ( $_POST['integrate-dcash-with-woocommerce-payment-id-nonce'] ?? '' ) ) );
		...
		if ( false === $updated ) {
			$text  = __( 'There was an issue completing the order. Please contact us as soon as possible to let us know about this issue. Your Order ID is:', 'integrate-dcash-with-woocommerce' ) . ' ' . $order_id . '. ';
